...
import kr.ac.hansung.cse.productmanager.entity.Product;
import kr.ac.hansung.cse.productmanager.service.ProductService;
...
public class ProductController {
    ...
    public String viewHomePage(Model model,
        ...
        if ("true".equals(loginSuccess)) {
            model.addAttribute("successMessage", user.getUsername() + "님, 환영합니다!");
        }
    ...
    public String saveProduct(@Valid @ModelAttribute("product") Product product, BindingResult bindingResult,
        ...
        if (bindingResult.hasErrors()) {
            // 에러가 있을 경우 다시 폼으로 (신규 등록인지 수정인지 구분)
